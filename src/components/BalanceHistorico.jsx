...
export default function BalanceHistorico() {
  ...
  const totalIngresos = balanceData.reduce((sum, m) => sum + Number(m.ingresos_totales || 0), 0)
  const totalCostos = balanceData.reduce((sum, m) => sum + Number(m.costos_refacciones || 0), 0)
  const totalGanancia = balanceData.reduce((sum, m) => sum + Number(m.ganancia_neta || 0), 0)
  const totalEntregas = balanceData.reduce((sum, m) => sum + Number(m.entregas_realizadas || 0), 0)

  if (loading) return (
    <div className="page">
      <div className="card"><p>Cargando balance histórico...</p></div>
    </div>
  )
  ...
  if (error) return (
    <div className="page">
      <div className="card">
        ...
        <Link to="/tickets" className="btn btn-secondary" style={{ marginTop: '1rem', display: 'inline-block' }}>
          ← Volver a Tickets
        </Link>
      </div>
    </div>
  ...
  return (
    <div className="page">
      <div className="page-header">
        <h1>Balance Histórico</h1>
        <Link to="/tickets" className="btn btn-secondary">← Volver a Tickets</Link>
      </div>
